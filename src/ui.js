...
export function createUI( { scene, sim, ants, env, sky, grass, props, foodballs, cones, editor, bees, godrays, cinematic, bench, music, spiders, colony, controls, camera, renderer, nestVolume, onReset, onSetColonyEnabled } ) {
	...
	const fBeeBehaviour = fPollinators.addFolder( 'Vol et comportement' );
	fBeeBehaviour.add( gfx, 'beeScoutRatio', 0, 0.6, 0.01 ).name( 'Part d’éclaireuses' );
	fBeeBehaviour.add( gfx, 'beeFlightAcceleration', 2, 50, 0.5 ).name( 'Accélération' );
	fBeeBehaviour.add( gfx, 'beeFlightFlutter', 0, 1.5, 0.02 ).name( 'Flottement multi-échelle' );
	fBeeBehaviour.add( gfx, 'beeForageDuration', 2, 40, 0.5 ).name( 'Butinage sur fleur (s)' );
	fBeeBehaviour.close();
	const fHoney = fPollinators.addFolder( 'Ruche et miel' );
	fHoney.add( gfx, 'beeHoneyMaturationSeconds', 10, 600, 5 ).name( 'Maturation miel (s)' );
	fHoney.add( gfx, 'beeInitialNectarStore', 0, 1000, 5 ).name( 'Nectar initial (reset)' );
	fHoney.add( gfx, 'beeInitialHoneyStore', 0, 5000, 10 ).name( 'Miel initial (reset)' );
	fHoney.add( gfx, 'beeInitialPollenStore', 0, 1000, 5 ).name( 'Pollen initial (reset)' );
	fHoney.close();
