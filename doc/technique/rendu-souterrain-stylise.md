...
Les mottes utilisent un matériau Lambert ; les trois objets GLB utilisent un `MeshStandardNodeMaterial` PBR rugueux, sans multiplication de teinte par instance. Tous partagent `sampleSDFClean(positionWorld) >= 0` via leur `maskNode`. Le test de profondeur découpe l’excavation et ce masque retire chaque fragment situé dans le vide physique du nid. Les directions candidates restent déterministes, puis le runtime projette les objets vers la plus petite enveloppe possible de la coque : ils sont bien visibles même dans un creux du relief. **Exposition** décale leur centre vers la caméra et peut autoriser un léger flottement stylistique, sans créer de mesh ni de draw supplémentaire.
...
Un raymarch opaque unique couvre tous les rayons de la vue souterraine ; aucune sphère de fond séparée n’est dessinée. La palette et le grain sont évalués au point monde touché. Une lumière ambiante neutre et une lumière ponctuelle ivoire proche de la caméra éclairent les objets sans filtre brun ; leurs couleurs UI restent exactes et leurs facettes conservent un relief lisible.
