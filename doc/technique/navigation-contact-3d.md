...
Le graphe consomme le bake versionné `natural-growth-tree-v2`, composé de 96 fiches déterministes. Une fiche contient sa position horizontale, sa fraction de profondeur, son parent strictement antérieur, son rôle et son éventuel détour de corridor. Le préfixe K24 est actif au démarrage normal ; K96 décrit le nid développé. Activer un préfixe plus long ne déplace aucun nœud et ne reconnecte aucune arête existante.

La topologie forme un seul arbre binaire enraciné. Les quatre fiches fondatrices constituent le premier tronc fonctionnel — garde, grenier, crèche et chambre royale — puis la croissance alterne prolongements, bifurcations et feuilles terminales. Un nœud a au plus deux enfants, ce qui évite les moyeux géométriques à nombreuses branches. La profondeur topologique et les longueurs de corridor sont volontairement variées et bornées par les oracles de validation.

Les nœuds ont trois rôles. `FUNCTIONAL` réserve les quatre fonctions fondatrices, `CHAMBER` crée une véritable salle biologique et `TRANSIT` crée un petit vestibule de passage exposé comme `ROOM.TUNNEL`. Cette hiérarchie de taille évite qu’une grosse salle apparaisse à chaque changement de direction. Elle remplace l’ancienne organisation en séries répétées de quatre unités et quatre strates, qui conservait un motif artificiel malgré la courbure locale des tunnels.

Un nœud volumique est creusé par l’union de trois ellipsoïdes tronqués, asymétriques et contenus dans une enveloppe conservatrice. Un tunnel emploie un axe à plusieurs harmoniques de basse fréquence et un rayon qui varie doucement sur seize capsules. Des détours latéraux et, exceptionnellement, verticaux sont contenus entre les collerettes des nœuds afin de contourner une structure étrangère sans créer de boucle. Les extrémités restent exactement celles du graphe, la position et la pente des collerettes restent nulles, et le rayon ne descend jamais sous la clearance minimale.
...
Le compilateur de pistes et le bake volumique partagent la même macro-géométrie organique propre : trois lobes tronqués par nœud, axe sinueux et rayon variable par capsule. La projection CPU sert d’oracle analytique ; le volume GPU en est la représentation échantillonnée. Seul le bruit de détail non contractuel est neutralisé sous les contacts afin de ne jamais retirer la surface située sous les pattes.
...
- `NEST-NATURAL-001` à `NEST-NATURAL-006` protègent respectivement la consommation du bake `natural-growth-tree-v2`, son déterminisme append-only à K24/K96, l’arbre binaire avec bifurcations/feuilles/profondeur, la variété des longueurs sans gabarit de quatre unités, la distinction entre vestibules et chambres tout en conservant les quatre fondateurs, puis le gel profond et l’empreinte SHA-256 du registre relu.
- `NEST-ORGANIC-001` à `NEST-ORGANIC-005` protègent la couverture des directions, les rotations dans les deux sens et la rareté des angles droits, puis les trois lobes bornés, la sinuosité visible mais limitée, les profils de rayon lisses sans rétrécissement et la cohérence de profondeur.
