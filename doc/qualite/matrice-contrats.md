...
| Laboratoire caméléon isolé : route dynamique exclusive ; géométrie source exacte avec `original_tail_vertices = 7206` ; douze os de queue sur ligne centrale courbe et poids géodésiques ; un seul corps dynamique Rapier et collider composé ; quatre appuis fixes ; pose procédurale corps entier, IK proximale à cinq itérations et angles anatomiques bornés ; queue passive XPBD de treize nœuds avec collisions ; mode **Physique libre** sans ragdoll instable ; pas à 120 Hz, interpolation et rejet des poses non finies. Les transitions multi-surfaces, la qualité de la démarche et la déformation de peau restent des preuves runtime. | `test/chameleon-lab-route.test.js`, `test/chameleon-lab-physics-world.test.js`, `test/chameleon-lab-controller.test.js`, `test/chameleon-lab-whole-body-gait.test.js`, `test/chameleon-lab-passive-tail.test.js`, `test/chameleon-lab-active-ragdoll.test.js` (`CHAMELEON-LAB-RAGDOLL-001` à `009`), `test/chameleon-physical-asset.test.js` |
