...
| Laboratoire caméléon isolé : route dynamique exclusive ; mesh `3.5.0` et anatomie `2.1.0` sur la géométrie source exacte avec `original_tail_vertices = 7206` ; aucun poids `tail_*` sur le corps, pont sacré rigide `tail_01..03`, dynamique `tail_04+` et rebase visuel ; axes épaule/coude/poignet et hanche/genou/cheville contenus dans le volume fermé ; un seul corps dynamique Rapier et collider composé ; quatre chaînes analytiques à ceinture mobile et appuis zygodactyles complets ; membres XPBD à faible tonus, capsules corps et auto-collision segmentaire uniquement en saisie/mode libre ; une projection de scène par nœud libre et par pas ; queue XPBD de treize nœuds avec sommeil sans jitter ; récupération du support d’impact sur mur/cylindre ; pas à 120 Hz, interpolation et rejet des poses non finies. Les transitions multi-surfaces, la qualité de la démarche et la silhouette de peau restent des preuves runtime. | `test/chameleon-lab-route.test.js`, `test/chameleon-lab-physics-world.test.js`, `test/chameleon-lab-controller.test.js`, `test/chameleon-lab-anatomical-limb.test.js`, `test/chameleon-lab-whole-body-gait.test.js`, `test/chameleon-lab-passive-limbs.test.js`, `test/chameleon-lab-passive-tail.test.js`, `test/chameleon-lab-active-ragdoll.test.js` (`CHAMELEON-LAB-RAGDOLL-001` à `016`), `test/chameleon-lab-hybrid-controller-allocation.test.js`, `test/chameleon-physical-asset.test.js` |
