...
| `BUTTERFLY-SIM` | Cycle œuf→larve→chrysalide→adulte, immatures invisibles, activité adulte météo-dépendante, vieillissement continu, fleurs partagées, SoA de 64 slots, quatre candidates fixes et draw VAT unique | `BUTTERFLY-SIM-001` à `014` | Inspection WebGPU : orientation, éclairage, teinte, visite florale, disparition/réapparition entre générations et masquage sous terre |
...
| Cycle complet des papillons, déterminisme, météo indépendante du vieillissement, ciblage borné, SoA, asset/clip, VAT, chargement paresseux, UI et draw unique | `test/butterfly-simulation.test.js`, `test/butterfly-integration.test.js` |
...
Une modification d’un des huit contrats est livrable lorsque :
