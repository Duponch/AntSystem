...
| Laboratoire caméléon isolé : route dynamique exclusive ; géométrie source exacte avec `original_tail_vertices = 7206` ; un seul corps dynamique Rapier et collider composé ; quatre appuis de pieds en tableaux fixes ; cadre de support, efforts et couples amortis et bornés ; IK restaurée depuis la pose de repos, quatre itérations et angles anatomiques limités ; mode **Physique libre** sans ragdoll articulé ; pas à 120 Hz, interpolation stable, plafond de rattrapage et rejet des poses non finies. Les transitions multi-surfaces, la silhouette de la queue et le ressenti restent des preuves runtime. | `test/chameleon-lab-route.test.js`, `test/chameleon-lab-physics-world.test.js`, `test/chameleon-lab-controller.test.js`, `test/chameleon-lab-active-ragdoll.test.js` (`CHAMELEON-LAB-RAGDOLL-001` à `007`), `test/chameleon-physical-asset.test.js` (`CHAMELEON-PHYSICAL-ASSET-001/002`) |
