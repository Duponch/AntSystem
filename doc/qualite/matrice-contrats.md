...
| `CHAMELEON-SIM` | Chasse transactionnelle ; graphe CSR global d’au plus 8 192 nœuds couvrant terrain, rochers, souches, troncs, branches, arbres verticaux et transitions ; exploration locale déterministe sans A* de routine ; corridor actif d’au plus 384 échantillons ; repère tangente/normale et quatre contacts approximatifs sans IK complet ; camouflage perceptif immobile par capture viewport unique, pigments éclairés, motifs stables, correspondance ≤ 0,86, contour et ombre résiduels, variantes préchauffées, profondeur conservée et zéro copie hors effet ; aucun draw/compute supplémentaire ; sélection et portée de debug | `CHAMELEON-SIM-001` à `037`, `CHAMELEON-SURFACE-001` à `005`, `WILDLIFE-INSPECTOR-001` à `003` | Inspection WebGPU : continuité terrain→rocher→tronc→arbre, orientation, exploration variée, transition perceptive progressive sans invisibilité, fuite des papillons, contact et rétraction de langue |
...
| Prédation, animation, graphe global de surfaces, clearance, corridors continus, exploration locale déterministe, camouflage perceptif borné à capture viewport partagée, motif objet stable, ombre résiduelle et absence de travail géométrique dans la boucle chaude | `test/chameleon-simulation.test.js`, `test/chameleon-track.test.js`, `test/chameleon-integration.test.js`, `test/chameleon-predation.test.js`, `test/chameleon-final-integration.test.js`, `test/chameleon-facing.test.js`, `test/chameleon-surface-graph.test.js`, `test/chameleon-camouflage.test.js` |
